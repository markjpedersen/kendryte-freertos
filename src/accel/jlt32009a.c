...
void init_rst(void) {
    ...
    configASSERT(gio);  // Should've been opened by init_dcx()
    gpio_set_drive_mode(gio, RST_GPIONUM, GPIO_DM_OUTPUT);
    gpio_set_pin_value(gio, RST_GPIONUM, GPIO_PV_LOW);
    usleep(100000);
    gpio_set_pin_value(gio, RST_GPIONUM, GPIO_PV_HIGH);
    usleep(100000);
}
...
void spi_control_init()
{
    ...
    spi_dev_set_clock_rate(spi_dfs32, SPI_LOW_CLOCK_RATE);
}

void tft_hard_init(void)
{
    init_dcx();
    init_rst();		// For DAN board.
    spi_control_init();
}
